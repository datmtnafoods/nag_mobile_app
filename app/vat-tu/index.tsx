  ...
  useEffect(() => {
    ...
    if (action === 'nhap' || action === 'ban') {
      ...
      if (!canCreateReceipt(perms, action)) {
        Alert.alert(
          'Thiếu quyền',
          `Bạn không có quyền tạo phiếu ${RECEIPT_KIND_META[action].label.toLowerCase()}.`,
        );
        return;
      }
      startReceiptDraft(action);
      router.push(
        (action === 'nhap' ? '/vat-tu/nhap-kho/new' : '/vat-tu/ban-hang/new') as never,
      );
    }
  }, [action, perms, startReceiptDraft]);
