...
- **BE `GET /kho/phieu-ban` chưa lọc được**: `svc.listPhieuBan()` không nhận tham số, `core/list.js` chỉ cắt offset/limit ⇒ màn "Lịch sử mua vật tư" của hộ đang **lọc client-side** theo `partyId` (tải cả trang rồi lọc). Cần BE thêm filter `partyId` để khỏi kéo thừa.
- **Lệch shape list kho**: mobile mong `Paginated{data,meta}`, BE trả `{rows,total}` (`core/list.js`) — ảnh hưởng **mọi màn list kho ở real mode**, chưa xử lý, cần một đợt riêng.
